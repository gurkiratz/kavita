 ...
 */
const DEFAULT_BASE = 'https://pub-4730074c0ff04638961b0cf1c2d55537.r2.dev';

/**
 * Both URLs can be overridden for local testing without editing this file — point
 * them at `wrangler dev` to exercise the app against a throwaway collection:
 *
 *   EXPO_PUBLIC_POEMS_URL=http://localhost:8787/poems \
 *   EXPO_PUBLIC_SCANS_URL=http://localhost:8787/scans npx expo start --web
 *
 * Expo inlines EXPO_PUBLIC_* at build time, so a production build with neither set
 * gets exactly the constants below.
 */
export const REMOTE_POEMS_URL =
  process.env.EXPO_PUBLIC_POEMS_URL || `${DEFAULT_BASE}/poems.json`;
export const REMOTE_SCANS_BASE_URL =
  process.env.EXPO_PUBLIC_SCANS_URL || `${DEFAULT_BASE}/scans`;
